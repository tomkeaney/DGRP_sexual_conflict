...
DGRP_data %>% 
  ...
  select(-c(Pooled, Trait_ID, Trait_nice, study_ID)) %>% 
    select(Trait, line, Sex, trait_value, everything())
)
```
...
We use data compiled in the _DGRP database_ (Belonte, Keaney, _et al_. 2023).
...
trait_data <-
  DGRP_data %>%  filter(!str_detect(Trait, "fitness"),
                        `# lines measured` >= 80 &
                          ...
                          !str_detect(Trait, "dopamine.response.to.paraquat.2021.m")) %>% # data for this trait was entered into the database incorrectly  
